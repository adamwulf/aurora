...
class module_bootstrap_os_overview_new_comments_gui extends module_bootstrap_module{
	...
	function run($data = false){
		...
		if(is_array($data->data())){
			...
			$this_week->add(new Text("<b>New Comments</b>"), $section_title_style);
			...
			foreach($comment_list as $comment){
				...
				$cal = $strongcal->getCalendarFromDb($comment["cal_id"]);
				$event = $cal->getEvent($comment["event_id"]);
				$e_title = $event->getDisplayValue("title");
				if(strlen($e_title) == 0){
					$e_title = "<i>no title</i>";
				}
				...
				$title_panel = new GridPanel(4);
				$title_panel->getStyle()->setClassname("overview_cell_style");
				$title_panel->getCellStyle()->setPaddingLeft(3);
				$title_panel->add($color);
				$title = new Link($title, "?view=event&cal_id=" . $cal->getId() . "&event_id=" . $event->getId());
				$title_panel->add($title);
				$title_panel->add(new Text(" for "));
				$e_title = new Link($e_title, "?view=event&cal_id=" . $cal->getId() . "&event_id=" . $event->getId());
				$title_panel->add($e_title);

				if($event->getDisplayValue("description") || count($event->comments()) || $event->isAllDay()){
					$tip = StrongcalGuiHelper::createEventTip($this->avalanche, $event);
					$menu_action = new ToolTipAction($e_title, $tip);
	...
	private function getNewStuff(){
		...
		if(!$this->avalanche->loggedInHuh()){
			// if not logged in, get new comments since yesterday...
