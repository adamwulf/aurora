...
class module_bootstrap_os_overview_new_events_gui extends module_bootstrap_module{
	...
	function run($data = false){
		...
		if(is_array($data->data())){
			...
			$this_week->add(new Text("<b>New Events</b>"), $section_title_style);
			...
			$total_height = 0;
			$event_list = $this->getNewStuff();
			$has_events = count($event_list);

			// keep track of the series that we've already
			// added, so we don't add multiple entries for a series
			$list_of_series = new HashTable();

			foreach($event_list as $event){
				if(!is_object($event->stealRecurrance()) ||
				    is_object($event->stealRecurrance()) && !$list_of_series->get($event->stealRecurrance()->getId())){
					...
					// if it's a series
					if(is_object($event->stealRecurrance())){
						$title .= "<span style='font-size: 75%;'>&nbsp;&nbsp;(series)</span>";
					}
					$color = new Panel();
	...
	private function getNewStuff(){
		...
		if(count($results)){
			...
			// each data type is a different key basically
			// and value is the list of results for that type
			foreach($results as $key => $value){
				$sorter = new MDASorter();
				$comp   = new StrongcalEventAddedComparator();
