...
//				$module_list .= $module_button->execute($html_visitor);
			...
			************************************************************************/
			...
			if(is_object($this->avalanche->ACCOUNTOBJ())){
				$purchase_link->addAction(new LoadPageAction("http://calendarfamily.net/member.php?subview=purchase&subsubview=upgrade&account=" . $this->avalanche->ACCOUNTOBJ()->name(), "_new"));
			}else{
				$purchase_link->addAction(new LoadPageAction("http://calendarfamily.net/member.php", "_new"));
			}
			...
			$icon = new Button("menu");
			$icon_style = new Style("loginButton xTrigger");
			$icon_style->setMarginLeft(2);
			$icon_style->setMarginRight(4);
