...
class module_bootstrap_strongcal_change_validation_gui extends module_bootstrap_module{
	...
	function run($data = false){
		...
		if(is_array($data->data())){
			...
			try{
				...
				if(isset($data_list["new_val"]) && isset($data_list["subsubview"]) && "validation" == $data_list["subsubview"]){
					...
					foreach($team_ids as $team_id){
						...
						if($group->type() == avalanche_usergroup::$PUBLIC || $group->type() == avalanche_usergroup::$USER ||
							...
							if($new_val == "r" || $new_val == "rw" || $new_val == ""){
								if(!$main_cal_obj->canReadName(array($group))){
									$main_cal_obj->updatePermission("name", "r", $group->getId());
								}
								$main_cal_obj->updatePermission("validation", $new_val, $group->getId());
							}else{
								throw new IllegalArgumentException("new_val must be either r, rw, or the empty string");
							}
						}
					}
					throw new RedirectException("index.php?view=manage_cals&subview=share&subsubview=validation&cal_id=" . $main_cal_obj->getId());
				}
			...
			**/
			...
			$hide_form->setName("validation_hide");
			$busy_form = new FormPanel("index.php");
			$busy_form->setName("validation_show");
			$read_form = new FormPanel("index.php");
			$read_form->setName("validation_add");
			
			$button = new Button("<");
			$button->setStyle(new Style("sharing_button"));
			$b = clone $button;
			$b->addAction(new SetValueAction($new_val_2, ""));
			$b->addAction(new ManualAction("xGetElementById(\"" . $busy_form->getName() . "\").submit();"));
			$busy_b_panel->add($b);
			$b = clone $button;
			$b->addAction(new SetValueAction($new_val_3, "r"));
			...
			$button->setStyle(new Style("sharing_button"));
			$b = clone $button;
			$b->addAction(new SetValueAction($new_val_1, "r"));
			$b->addAction(new ManualAction("xGetElementById(\"" . $hide_form->getName() . "\").submit();"));
			$hide_b_panel->add($b);
			$b = clone $button;
			$b->addAction(new SetValueAction($new_val_2, "rw"));
			$b->addAction(new ManualAction("xGetElementById(\"" . $busy_form->getName() . "\").submit();"));
			$busy_b_panel->add($b);
			...
			foreach($groups as $group){
				if($group->type() == avalanche_usergroup::$PUBLIC || $group->type() == avalanche_usergroup::$USER && $group->getId() != -$main_cal_obj->author() ||
					   $group->type() == avalanche_usergroup::$PERSONAL && $group->author() == $this->avalanche->getActiveUser()){
					if($main_cal_obj->canWriteValidations(array($group))){
					...
					}else if($main_cal_obj->canReadValidations(array($group))){
						$entry = new BorderPanel();
						$entry->setStyle(new Style("sharing_item_style"));
						$entry->getStyle()->setHandCursor();
						$name = new Panel();
			...
			$temp_panel->add(new Text("<b>Moderator</b>"), $border_cell_style);
			$temp_panel->add(new Text("<b>Manage Moderators</b>"), $normal_cell_style);
			$temp_panel->add($hide_panel, $border_cell_style);
			$temp_panel->add($busy_panel, $border_cell_style);
			$temp_panel->add($read_panel, $normal_cell_style);
			
			$title = new Text("Moderators and Event Validation");
			$title->setStyle(new Style("content_title"));
			$cal_info->add($title);
			$text = new Text("Here you can decide whether each groups are eligable to become moderators. You can also decide which groups can set up the moderation rules for this calendar. For a group to be eligable for this permission, the Event Transparency for that group must be set to at least \"Full Information.\"");
			$t = new Panel();
			$t->setValign("top");
			$t->add($text);
			$t->setWidth("100%");
	...
	private function setUpForm(FormPanel $form, $panel, $main_cal_obj, $new_val){
		...
		$form->addHiddenField("subsubview", "validation");
