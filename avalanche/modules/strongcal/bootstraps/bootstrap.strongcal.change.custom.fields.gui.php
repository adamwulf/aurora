...
class module_bootstrap_strongcal_change_custom_fields_gui extends module_bootstrap_module{
	...
	function run($data = false){
		...
		if(is_array($data->data())){
			...
			try{
				...
				if(isset($data_list["new_val"]) && isset($data_list["subsubview"]) && "field" == $data_list["subsubview"]){
					...
					foreach($team_ids as $team_id){
						...
						if($group->type() == avalanche_usergroup::$PUBLIC || $group->type() == avalanche_usergroup::$USER ||
							...
							if($new_val == "rw" || $new_val == ""){
								if(!$main_cal_obj->canReadName(array($group))){
									$main_cal_obj->updatePermission("name", "r", $group->getId());
								}
								$main_cal_obj->updatePermission("field", $new_val, $group->getId());
							}else{
								throw new IllegalArgumentException("new_val must be either r, rw, or the empty string");
							}
						}
					}
					throw new RedirectException("index.php?view=manage_cals&subview=share&subsubview=field&cal_id=" . $main_cal_obj->getId());
				}
			...
			**/
			...
			$hide_form->setName("field_hide");
			$busy_form = new FormPanel("index.php");
			$busy_form->setName("field_manage");
			...
			$temp_panel->add(new Text("<b>Manage Custom Fields</b>"), $normal_cell_style);
			$temp_panel->add($hide_panel, $border_cell_style);
			$temp_panel->add($busy_panel, $normal_cell_style);
			
			$title = new Text("Custom Field Management");
			$title->setStyle(new Style("content_title"));
			$cal_info->add($title);
			$text = new Text("Here you can decide which groups are allowed to modify this calendar's Add Event Form with custom fields. For a group to be eligable for this permission, the Event Transparency for that group must be set to at least \"Full Information.\"");
			$t = new Panel();
			$t->setValign("top");
			$t->add($text);
			$t->setWidth("100%");
	...
	private function setUpForm(FormPanel $form, $panel, $main_cal_obj, $new_val){
		...
		$form->addHiddenField("subsubview", "field");
