...
class module_bootstrap_strongcal_weekview_gui extends module_bootstrap_module{
	...
	function run($data = false){
		...
		if(is_array($data->data())){
			...
			$last = strtotime("-1 week", $first_of_week);
			$next = strtotime("+1 week", $first_of_week);
			$last = "index.php?view=week&date=" . date("Y-m-d",$last);
			$next = "index.php?view=week&date=" . date("Y-m-d",$next);
