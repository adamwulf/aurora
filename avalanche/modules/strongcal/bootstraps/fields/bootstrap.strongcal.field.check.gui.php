...
class module_bootstrap_strongcal_field_check_gui extends module_bootstrap_module{
		...
		if(is_array($data->data())){
			...
			************************************************************************/
			...
			if($main_cal_obj->canWriteFields()){
				$field = $strongcal->fieldManager()->getField("check");
				...
				if(isset($data_list["done"])){
					...
					$to_add["value"] = isset($data_list["default_value"]) ? "1" : "";
					try{
						if(is_object($field_from_cal)){
							...
							if($main_cal_obj->addField($to_add)){
								...
								$cal_info = new ErrorPanel(new Text("could not create Checkbox"));
					...
					$temp_panel->add(new Text("<br>You're almost done! Just pick the checkbox's default value below.<br><br>"));
					$cal_info->add($temp_panel);

					$check_default = new GridPanel(2);
					$check_default->getCellStyle()->setPadding(3);
					...
					$field_default->getStyle()->setMarginTop(20);
					$field_default->getStyle()->setMarginBottom(20);
