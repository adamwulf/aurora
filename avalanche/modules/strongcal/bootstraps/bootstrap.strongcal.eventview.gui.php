	...
	function run($data = false){
		...
		if(is_array($data->data())){
			...
			if(is_object($event) && $cal->canReadEvent($event->getId())){
				...
				$username = $os->getUsername($event->author());
				$link = new Link(htmlspecialchars($username), "javascript:;");
				$this->createUserMenu($link, $event->author());
				$tmp = new Panel();
				$tmp->setStyle(new Style("event_author"));
				$tmp->add(new Text("Author: "));
				$tmp->add($link);
				$second_row->add($tmp);
	...
	private function getTabs($subview, $event_panel, $attendee_panel, $reminder_panel, $event){
		...
		if($subview == "event"){
			...
			if(isset($this->data_list["add_reminder"]) && $this->data_list["add_reminder"]){
				...
				//
				// ** either add the reminder to 1 event
				// ** or add it to all in the series
				//
				$events = array( $event );
				if(isset($this->data_list["add_to_all"]) && $this->data_list["add_to_all"]){
					if(is_object($event->stealRecurrance())){
				...
				foreach($events as $e){
					// add the reminder only if the event is in the future
					if($e->getDisplayValue("start_date") >= $today){
						$r = $reminder->addReminder();
						if(isset($this->data_list["all_attendees"]) && $this->data_list["all_attendees"]){
							$r->type(module_reminder_reminder::$TYPE_EVENT_ATTENDEES, $e);
						}else{
							$r->type(module_reminder_reminder::$TYPE_EVENT, $e);
						}
						$r->day((int)$this->data_list["days_before"]);
						$r->hour((int)$this->data_list["hours_before"]);
						$r->minute((int)$this->data_list["minutes_before"]);

						if(isset($this->data_list["user_id"]) && is_array($this->data_list["user_id"])){
							$list = $this->data_list["user_id"];
							foreach($list as $user_id){
								if($user_id != $r->author()){
									$r->addUser((int)$user_id);
								}
							}
						}
						...
						$r->subject($subject->getValue());

						$body = new SmallTextInput();
						$body->setName("body");
						$body->loadFormValue($this->data_list);
						$r->body($body->getValue());
					}
				}
				//
				// ** end loop for adding reminders
				//


				header("Location: index.php?view=$view&subview=$subview&cal_id=" . $this->cal_id . "&event_id=" . $this->event_id . "&reminder_added=" . $r->getId());
				exit;
			}
		...
		if($event->calendar()->canReadEvent($event->getId()) ||
		   $this->avalanche->loggedInHuh() == $event->author()){
			...
			if(is_object($event->stealRecurrance())){
				...
				$add_to_all = new CheckInput("Add reminder to all events in the series");
				$add_to_all->setName("add_to_all");
				$add_to_all->setValue("1");
				$add_to_all->setChecked(true);
				$add_to_all->getStyle()->setMarginBottom(5);
				$form_body->add($add_to_all);
			}
