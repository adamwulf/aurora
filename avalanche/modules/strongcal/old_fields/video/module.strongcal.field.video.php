...
//////////////////////////////////////////////////////////////////
class module_strongcal_field_video extends module_strongcal_field {
	...
	function display_value($cal_id=false, $event_id=false){
		...
		if($this->_value){
			...
			if($val){
				$to_ret = $filename . " ($size)";
				$to_ret .= "<br><a href='" . $this->_path . "video/downloadvideo.php?cal_id=" . $cal_id . "&event_id=$event_id&field=" . $this->field() . "' target='download_frame'>download</a>";
				$to_ret .= "<br><a href='#' onClick='window.open(\"" . $this->_path . "video/viewvideo.php?cal_id=" . $cal_id . "&event_id=$event_id&field=" . $this->field() . "\", \"_new\",\"width=415,height=325;\")'>view</a>";
				return $to_ret;
			}else{
				return "";
			}
