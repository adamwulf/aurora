...
if ($step_num == 1) {
	...
	$mid_row = $skin->td("<img src='phpavalanche.jpg' height='150' width='300'>", " height='150' width='300'");
...
if ($step_num == 5) {
	...
	$sql_pass_input = $skin->input("type=password size=32 name='sql_passname' value='$sql_passname'");
...
if ($step_num == 6) {
	$sql = "UPDATE " . $sql_prefix . "users SET username='" . $username . "', password='" . $password . "' WHERE id='1'";
...
if ($step_num == 7) {
	...
	$flag = 0;
	if(!$root_directory){
		$root_directory = substr($_SERVER['PATH_TRANSLATED'], 1, strrpos($_SERVER['PATH_TRANSLATED'], $_SERVER['SCRIPT_NAME']));
		$flag = 1;
	}
	if(!$host){
		if($flag == 1){
	...
	$on = true;
	if(($flag == 3) && $on){
		// fix the root directory and at_path here.
		$temp_root = array();
		$temp_root = explode("/", $root_directory);
		$temp_host = array();
		$temp_host = explode("/", $host);
		$temp_path = array();
		$temp_path = explode("/", $at_path);
		$temp_root = array_reverse($temp_root);
		$temp_host = array_reverse($temp_host);
		$temp_path = array_reverse($temp_path);

		$keep_going = true;
		$temp2_host = array();
		for($i=0;($i<count($temp_path));$i++){
			if(($temp_path[$i] == $temp_root[$i]) && $keep_going){
				$temp_root[$i] = "";
			}else{
				$keep_going = false;
				$temp2_host[] = $temp_path[$i];
				$temp_path[$i] = "";
			}
		}
		$temp_root = array_filter($temp_root, "not_null_str");
		$temp_host = array_filter($temp_host, "not_null_str");
		$temp_path = array_filter($temp_path, "not_null_str");

		$temp2_host = implode("/", array_reverse($temp2_host));
		$temp_root = implode("/", array_reverse($temp_root));
		$temp_host = implode("/", array_reverse($temp_host));
		$temp_path = implode("/", array_reverse($temp_path));

		$root_directory = "/" . $temp_root . "/";
		if($temp2_host){
			$temp2_host .= "/";
		}
		$host = "http://" . $temp_host . "/" . $temp2_host;
		$at_path = $temp_path . "/";
	}
...
if ($step_num == 8) {
	...
	$str .= " include_once ROOT . APPPATH . INCLUDEPATH . \"include.php\";\n";
	$str .= "}\n";
	$str .= "?>\n";
...
if ($step_num == 10) {
	...
	$backText = $skin->font(". back .");
	$next_button = $skin->a($nextText, "href='" . $host . $at_path . "command.php'");
...
echo "<head>";
echo "<style>";
include "skins/installer/skin.installer.style.css";
echo "</style>";
...
echo "\n}";
include "skins/installer/skin.installer.java.js";
echo "</script>";
echo "<center>";
echo $everything;
echo "</center>";
