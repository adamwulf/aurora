...
//////////////////////////////////////////////////////////////////
class module_strongcal_field_phone extends module_strongcal_field {
	...
	function load_form_value($prefix, $my_array){
		...
		$this->_value = $area_val . "-" . $pre_val . "-" . $post_val;
		return true;
	}
	...
	function toHTML($prefix, $skin, $override=false){
		...
		$area_val = substr($value,0,3);
		$pre_val = substr($value,4,3);
		$post_val = substr($value,8,4);

		if(!ereg("^([0-9])+$", $area_val)){        //!is_integer($area_val+0)){
			$area_val = "";
		}
		if(!ereg("^([0-9])+$", $pre_val)){
			$pre_val = "";
		}
		if(!ereg("^([0-9])+$", $post_val)){
			$post_val = "";
		}


		$hidden = $skin->input("type='hidden' name='input_type' value='phone'");
		...
		$post  = "<input name='$prefix" . $this->field() . "_post' value='$post_val' style='" . $skin->form_style() . "  BACKGROUND-COLOR: " . $skin->form_color() . "; border-width: 0px;' size='4' maxlength='4'>";
		$phone = $buffer->table($buffer->tr($buffer->td($skin->font($area . "-" . $pre . "-" . $post))), "cellpadding='0' cellspacing='0' style='" . $skin->form_border_style() . "  BACKGROUND-COLOR: " . $skin->form_color() . "; border-width: 1px;'");
		return $phone;
	}
