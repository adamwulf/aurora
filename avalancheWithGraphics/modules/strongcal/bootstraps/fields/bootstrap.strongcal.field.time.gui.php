...
class module_bootstrap_strongcal_field_time_gui extends module_bootstrap_module{
		...
		if(is_array($data->data())){
			...
			************************************************************************/
			...
			if($main_cal_obj->canWriteFields()){
				$field = $strongcal->fieldManager()->getField("time");
				...
				if(isset($data_list["done"])){
					...
					try{
						if(is_object($field_from_cal)){
							...
							if($main_cal_obj->addField($to_add)){
								...
								$cal_info = new ErrorPanel(new Text("could not create Time field"));
					...
					$temp_panel->add(new Text("<br>You're almost done! Just click the Finish button below to add your new Time field.<br><br>"));
