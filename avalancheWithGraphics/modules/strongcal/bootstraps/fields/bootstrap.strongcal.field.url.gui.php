...
class module_bootstrap_strongcal_field_url_gui extends module_bootstrap_module{
		...
		if(is_array($data->data())){
			...
			************************************************************************/
			...
			if($main_cal_obj->canWriteFields()){
				$field = $strongcal->fieldManager()->getField("url");
				...
				if(isset($data_list["field_name"])){
					...
					throw new Exception("field_name not defined in form input");
				}
				if(isset($data_list["done"])){
					...
					$gui->getStyle()->setFontFamily("verdana, sans-serif");
					$gui->getStyle()->setFontSize(9);
