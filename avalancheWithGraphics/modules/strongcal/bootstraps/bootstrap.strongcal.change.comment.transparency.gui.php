...
class module_bootstrap_strongcal_change_comment_transparency_gui extends module_bootstrap_module{
	...
	function run($data = false){
		...
		if(is_array($data->data())){
			...
			try{
				...
				if(isset($data_list["new_val"]) && isset($data_list["subsubview"]) && "comment" == $data_list["subsubview"]){
					...
					foreach($team_ids as $team_id){
						...
						if($group->type() == avalanche_usergroup::$PUBLIC || $group->type() == avalanche_usergroup::$USER ||
							...
							if($new_val == "r" || $new_val == "rw" || $new_val == ""){
								...
								$main_cal_obj->updatePermission("comments", $new_val, $group->getId());
							}else{
								throw new IllegalArgumentException("new_val must be either r, rw, or the empty string");
							}
						}
					}
					throw new RedirectException("index.php?view=manage_cals&subview=share&subsubview=comment&cal_id=" . $main_cal_obj->getId());
				}
			...
			**/
			...
			$hide_form->setName("comment_hide");
			$busy_form = new FormPanel("index.php");
			$busy_form->setName("comment_show");
			$read_form = new FormPanel("index.php");
			$read_form->setName("comment_add");
			...
			foreach($groups as $group){
				if($group->type() == avalanche_usergroup::$PUBLIC || $group->type() == avalanche_usergroup::$USER && $group->getId() != -$main_cal_obj->author() ||
					   $group->type() == avalanche_usergroup::$PERSONAL && $group->author() == $this->avalanche->getActiveUser()){
					if($main_cal_obj->canWriteComments(array($group))){
					...
					}else if($main_cal_obj->canReadComments(array($group))){
						$entry = new BorderPanel();
						$entry->setStyle(new Style("sharing_item_style"));
						$entry->getStyle()->setHandCursor();
						$name = new Panel();
			...
			$temp_panel->add(new Text("<b>Hide Comments</b>"), $border_cell_style);
			$temp_panel->add(new Text("<b>Show All</b>"), $border_cell_style);
			$temp_panel->add(new Text("<b>Add/Edit/Del Comments</b>"), $normal_cell_style);
			...
			$title = new Text("Comment Transparency");
			$title->setStyle(new Style("content_title"));
			$cal_info->add($title);
			$text = new Text("Here you can decide whether each user or group is allowed to read and write calendar comments. For a user or group to be eligable for this permission, the Event Transparency for that user or group must be set to at least \"Show All.\"");
			$t = new Panel();
			$t->setValign("top");
			$t->add($text);
			$t->setWidth("100%");
	...
	private function setUpForm(FormPanel $form, $panel, $main_cal_obj, $new_val){
		...
		$form->addHiddenField("subsubview", "comment");
