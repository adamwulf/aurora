...
class AccountMain extends module_bootstrap_module{
	...
	private function getPurchase($data_list){
		...
		if($subview == "overview"){
			...
			$title->add(new Link("upgrade", "member.php?account=" . $this->account->name() . "&subview=purchase&subsubview=upgrade&testserver=" . (isset($data_list["testserver"]) ? $data_list["testserver"] : "")));
			$title->add(new Text(" ]"));
			...
			$title = new Panel();
			$title->setStyle($title_details_style);
			$title->add(new Text("Upgrade Plan [ "));
			...
			$left_form->add(new Text("I would like to upgrade to... "));
			...
			$subtotal_title->add(new Text("Potential New Plan"));
