...
class module_bootstrap_team_header_gui extends module_bootstrap_module{

	private $avalanche;
	private $doc;
	private $group;

	function __construct($avalanche, Document $doc, avalanche_usergroup $group){
	...
	function run($data = false){
		...
		if($data === false){
			...
			**/
			...
			$color_box->getStyle()->setBackground("#FFFFFF");
			
			// the calendar name
			$cal_info = new BorderPanel();
			$cal_info->setWest($color_box);
			$str = $main_group_obj->name();
			$cal_info->setCenter(new Text($str));
			$cal_info->setStyle($nobg);
			
			// the owner avatar
			$avatar = new Icon($this->avalanche->HOSTURL() . $this->avalanche->getAvatar($main_group_obj->author()));
			...
			$header->add(new Link($title, "index.php?view=manage_teams"));
			if(isset($main_group_obj) && is_object($main_group_obj)){
				// $header->add(new Text(" &gt;&gt; "));
				// $header->add(new Text($main_group_obj->name()));
			}
			$top_row->setCenter($header);
			
			// make username link
			$link = new Link($os->getUsername($main_group_obj->author()), "javascript:;");
			$link->getStyle()->setFontColor("black");
			$this->createUserMenu($link, $main_group_obj->author());
			...
			<td></td>
			<td height='7' width='648' colspan='2'></td>
			</tr>
			<tr>
			<td height='26' bgcolor='#ADB8D0' style='border-bottom:1px solid #8A9BB3;border-top:1px solid #8A9BB3;'>
			&nbsp;
			</td>
			<td bgcolor='#ADB8D0' style='border-bottom:1px solid #8A9BB3;border-top:1px solid #8A9BB3;'></td>
			<td height='26' bgcolor='#ADB8D0' id='ownedby' style='font-family: verdana; font-size: 12pt; border-bottom:1px solid #8A9BB3;border-top:1px solid #8A9BB3;'>
			&nbsp;&nbsp;" . $main_group_obj->name() . "
			</td>
			<td nowrap height='26' align='right' bgcolor='#ADB8D0' style='padding-left:18px;font-family: verdana; font-size: 9pt; border-bottom:1px solid #8A9BB3;border-top:1px solid #8A9BB3;'>
			by $link&nbsp;&nbsp;
			</td>
			</tr>
			<tr>
