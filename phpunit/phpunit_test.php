...
// {{{ SelfTestResult
/**
 * Specialize result class for use in self-tests, to handle special situation
 * where many tests are expected to fail.
 */
class SelfTestResult extends TextTestResult {
	...
	// {{{ _endTest()
	/**
	 * Report both the test result and, for this special situation where some
	 * tests are expected to fail, a "meta" test result which indicates whether
...
// }}}
// {{{ PrettySelfTestResult
/**
 * This is a prettified version of the self test result.
 *
 * Note that if you have any problems with this (parse errors) in earlier
 * version of PHP, please report them to author as it's been so long since
 * I've programmed in it, I forgot which syntax rules apply where.
 *
 * @author terry chay <tychay@alumni.caltech.edu>
 */
class PrettySelfTestResult extends PrettyTestResult {
	// {{{ __construct()
	/**
	 * Like the normal constructor but contains the meta result.
	 */
	function PrettySelfTestResult() {
		parent::TestResult();  // call superclass constructor
		echo '<h2>Tests</h2>',"\n",
    ...
    function _endTest($test) {
		$outcome = $test->failed()
				 ...
				 : ' class="Pass">OK';
		if ($test->errored()) {
			$outcome = ' class="Error">ERROR';
		}
		
		$expect_failure = preg_match('/fail/i', $test->name());
		$test_passed = ($test->failed() == 0);
	    $meta_outcome = ($expect_failure xor $test_passed)
				      ? ' class="Expected">as expected'
					  : ' class="Unexpected">UNEXPECTED';
		if ($test->errored()) {
	    	$meta_outcome = ' class="Unknown">unknown';
		}
...
if (phpversion() >= '4') {
	...
	function AssertEqualsTests4($name) { $this->TestCase($name); }
